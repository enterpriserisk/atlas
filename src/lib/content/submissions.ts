...
export async function createPlaybookSubmission(
  ...
  const rows = (await sql`
    INSERT INTO playbook_submissions
      (slug, title, category, tags, ai_tools_referenced, human_review_required, summary, body, contributor_label)
    VALUES
      (${slug}, ${input.title}, ${input.category}, ${input.tags}, ${input.aiToolsReferenced},
       ${input.humanReviewRequired}, ${input.summary}, ${input.body}, ${input.contributorLabel})
    RETURNING *
  `) as unknown as SubmissionRow[];
